...
The product-based version of COWAS predicts a simple interaction term between observed expression levels instead of predicting the interaction between their nonlinear genetic components. Here we plot performance metrics and association results for the product-based version of COWAS, which we applied to Alzheimer's disease.
...
ad_product_results <- merge(ad_product_results, ad_product_metrics, by = "ID")

ad_product_results <- ad_product_results[CORRELATION_A > 0.03 & CORRELATION_B > 0.03 & CORRELATION_CO > 0.03, ]

# Sort and save the association results
ad_product_results <- sort_results(ad_product_results)
fwrite(ad_product_results, file = "figures_tables/csv/results_ad_product.csv")

# Extract interesting results
product_bonferroni_pairs <- 0.05 / nrow(ad_product_results)
product_bonferroni_individual <- 0.05 / length(unique(c(ad_product_results$ID_A, ad_product_results$ID_B)))

sig_product_global <- ad_product_results[PVAL_FSTAT_JOINT < product_bonferroni_pairs, ]
sig_product_interaction <- ad_product_results[PVAL_THETA_JOINT_CO < product_bonferroni_pairs, ]

sig_product_pwas_a <- ad_product_results[PVAL_THETA_MARGINAL_A < product_bonferroni_individual, ]
sig_product_pwas_b <- ad_product_results[PVAL_THETA_MARGINAL_B < product_bonferroni_individual, ]

sig_product_pwas <- ad_product_results[PVAL_THETA_MARGINAL_A < product_bonferroni_individual | 
                                         PVAL_THETA_MARGINAL_B < product_bonferroni_individual, ]

sig_product_any <- ad_product_results[PVAL_FSTAT_JOINT < product_bonferroni_pairs | 
