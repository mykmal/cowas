...
option_list <- list(
  ...
  make_option(
    ...
    help = "The type of model to fit. Valid options are 'stepwise' (linear regression with
            forward stepwise variable selection by BIC), 'ridge' (linear regression
  ...
  make_option(
    ...
    type = "logical",
    default = TRUE,
    help = "Should expression levels be normalized using a rank-based inverse normal
            transformation before fitting imputation models? If FALSE, expression values
  ...
  make_option(
    ...
    type = "logical",
    default = TRUE,
    help = "Should models be trained for the product-based version of COWAS? If FALSE, models
            will instead be trained for the residual-based version of COWAS. In product-based
            COWAS, the outcome of the co-expression model is the product of observed expression
            levels. In residual-based COWAS, the outcome of the co-expression model is instead
            the product of single-protein model residuals. See our paper for details.
            [default: %default]"
  )
)
...
# Remove allele codes after each rsid if they are present
...
# Fill in missing dosages with the mode for each variant.
...
TrainStepwise <- function(z_a, z_b, z_both, x) {
  ...
  step_a <- stats::step(lm_null_a, scope = formula_full_a, direction = "forward",
                        k = log(nrow(data_a)), trace = 0)
  step_b <- stats::step(lm_null_b, scope = formula_full_b, direction = "forward",
                        k = log(nrow(data_b)), trace = 0)
  
  # Compute the product of observed expression levels if running product-based COWAS
  if (opt$product_based == TRUE) {
    x_co <- data_a$x_a * data_b$x_b
  } else {
    # Compute the product of single-protein model residuals if running residual-based COWAS
    x_co <- step_a$residuals * step_b$residuals
  }
  
  # Create a data table containing co-expression values and all variant genotypes
  ...
  step_co <- stats::step(lm_null_co, scope = formula_full_co, direction = "forward",
                         k = log(nrow(data_co)), trace = 0)
...
TrainGlmnet <- function(z_a, z_b, z_both, x, alpha) {
  ...
  # The glmnet package only accepts matrices and vectors as inputs
  z_a <- as.matrix(z_a)
  z_b <- as.matrix(z_b)
  z_both <- as.matrix(z_both)
  x <- as.matrix(x)
  
  # Fit a penalized regression model for protein_a
  penalized_a <- cv.glmnet(x = z_a, y = x[, 1],
                           family = "gaussian", type.measure = "mse",
                           alpha = alpha, nfolds = 5,
                           standardize = FALSE, intercept = TRUE,
                           parallel = use_cores)
  
  # Fit a penalized regression model for protein_b
  penalized_b <- cv.glmnet(x = z_b, y = x[, 2],
                           family = "gaussian", type.measure = "mse",
                           alpha = alpha, nfolds = 5,
                           standardize = FALSE, intercept = TRUE,
                           parallel = use_cores)
  ...
  if (opt$product_based == TRUE) {
    ...
    # Compute the product of single-protein model residuals if running residual-based COWAS
    pred_a <- predict(object = penalized_a, newx = z_a, s = "lambda.min", type = "response")
    pred_b <- predict(object = penalized_b, newx = z_b, s = "lambda.min", type = "response")
    x_co <- (x[, 1] - pred_a) * (x[, 2] - pred_b)
  }
  
  # Fit a penalized regression model to predict the two proteins' co-expression
  penalized_co <- cv.glmnet(x = z_both, y = x_co,
                            family = "gaussian", type.measure = "mse",
                            alpha = alpha, nfolds = 5,
                            standardize = FALSE, intercept = TRUE,
                            parallel = use_cores)
  ...
  # Store fitted model weights.
  # The intercept is ignored because it only captures non-genetic effects.
  model_a_weights <- coef(penalized_a, s = "lambda.min")[-1, ]
  model_b_weights <- coef(penalized_b, s = "lambda.min")[-1, ]
  model_co_weights <- coef(penalized_co, s = "lambda.min")[-1, ]
  
  return(list(weights_a = model_a_weights, weights_b = model_b_weights, weights_co = model_co_weights,
              model_a = penalized_a, model_b = penalized_b, model_co = penalized_co))
}

# Train and evaluate the prediction models --------------------------------------------------------

# Define training and test subsets
test_indices <- sample(x = n_expression, size = ceiling(0.2 * n_expression))

# Train the requested model type on the training set
if (opt$model == "stepwise") {
  training_output <- TrainStepwise(genotypes_a[!test_indices, ], genotypes_b[!test_indices, ],
                                   genotypes[!test_indices, ], expression[!test_indices, ])
} else if (opt$model == "ridge") {
  training_output <- TrainGlmnet(genotypes_a[!test_indices, ], genotypes_b[!test_indices, ],
                                 genotypes[!test_indices, ], expression[!test_indices, ],
                                 alpha = 0)
...
# Check that the predicted values on the test set are not constant
if (sd(imputed_test_a) <= 0 || sd(imputed_test_b) <= 0 || sd(imputed_test_co) <= 0) {
  stop("Imputed expression or co-expression is constant for at least one of the models in the pair ", opt$protein_a, " - ", opt$protein_b, ". This pair will be skipped.")
}

# Free up memory
rm(training_output)

# If running product-based COWAS, predictive performance can be evaluated before fitting full-sample models
if (opt$product_based == TRUE) {
  # Calculate the product of the two proteins' observed expression levels
  ...
  if (cor_test_a$estimate < opt$cor_threshold || cor_test_b$estimate < opt$cor_threshold || cor_test_co$estimate < opt$cor_threshold) {
    stop("At least one of the models in the pair ", opt$protein_a, " - ", opt$protein_b, " fails to reach the correlation threshold. This pair will be skipped.")
  }
}

# Fit full-sample models
if (opt$model == "stepwise") {
  full_output <- TrainStepwise(genotypes_a, genotypes_b, genotypes, expression)
} else if (opt$model == "ridge") {
  full_output <- TrainGlmnet(genotypes_a, genotypes_b, genotypes, expression, alpha = 0)
} else if (opt$model == "lasso") {
  full_output <- TrainGlmnet(genotypes_a, genotypes_b, genotypes, expression, alpha = 1)
} else if (opt$model == "elastic_net") {
  full_output <- TrainGlmnet(genotypes_a, genotypes_b, genotypes, expression, alpha = 0.5)
}
...
# If running residual-based COWAS, calculate the product of full-sample model residuals on the test set
# in order to evaluate the models' predictive performance
if (opt$product_based == FALSE) {
  # Get predicted values for the single-protein models on the full data set
  if (opt$model == "stepwise") {
  ...
  # Calculate the product of single-protein model residuals
  set(x = expression, j = "coexpression", value = (expression[[opt$protein_a]] - imputed_full_a) * (expression[[opt$protein_b]] - imputed_full_b))
  
  # Check that the predicted expression and co-expression values are not constant
...
# Write the performance metrics to the output file
output_path <- paste0(opt$out_folder, "/performance_metrics.tsv")
if (!file.exists(output_path)) {
  write.table(t(c("ID_A", "ID_B", "SAMPLE_SIZE",
                "NFEATURES_A", "CORRELATION_A", "PVAL_A", "R2_A",
                "NFEATURES_B", "CORRELATION_B", "PVAL_B", "R2_B",
                "NFEATURES_CO", "CORRELATION_CO", "PVAL_CO", "R2_CO")),
                file = output_path, quote = FALSE, sep = "\t", eol = "\n",
                row.names = FALSE, col.names = FALSE)
}
write.table(t(metrics), file = output_path, quote = FALSE, sep = "\t", eol = "\n",
